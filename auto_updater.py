...
def install_update(installer_path, cleanup_callback=None):
    ...
    try:
        ...
        # Create a batch script that waits for the app to fully close, then runs installer
        # This ensures all DLLs are released before the installer tries to update them
        batch_content = f'''@echo off
:: Wait for the application to fully exit (check if process is gone)
timeout /t 3 /nobreak >nul

:: Run the installer silently (no windows)
"{installer_path}" /VERYSILENT /CLOSEAPPLICATIONS /RESTARTAPPLICATIONS /SP-
