...
VOICEOVER_URL = "https://fish.audio/app/text-to-speech/?modelId=e4a2d14e7f5c4b2d80a6c56538051612&version=s1"  # Generic URL, model ID set by orchestrator
#VOICEOVER_URL = "https://fish.audio/text-to-speech/?modelId=c48162cbf1aa4732a961eb5fad38674c&version=s1"
...
class SmartTabProcessor:
    ...
    def find_elements(self):
        ...
        try:
            ...
            self.page.wait_for_function("""
                () => {
                    ...
                    for (const btn of buttons) {
                        ...
                        if ((text.includes('Generate speech') || text.includes('Generate') || text.includes('Synthesize'))
            ...
            # Find the generate button — use Playwright's text matching with last to skip duplicates
            generate_btn = self.page.locator("button", has_text="Generate speech").last
            try:
                if not generate_btn.is_visible():
                    generate_btn = self.page.locator("button", has_text="Generate").last
            except Exception:
                generate_btn = self.page.locator("button", has_text="Generate").last
            try:
                if not generate_btn.is_visible():
                    generate_btn = self.page.locator("button", has_text="Synthesize").last
            except Exception:
                pass
    ...
    def is_ready_for_new_task(self):
        ...
        try:
            ...
            # Fish Audio changed download from text button to icon button
            download_btn = self.page.locator('a[download], button[aria-label*="ownload" i], a[aria-label*="ownload" i]').first
            ...
            for text in ["Generate speech", "Generate", "Synthesize"]:
                try:
                    btn = self.page.locator("button", has_text=text).last
    ...
    def check_download_ready(self):
        ...
        try:
            ...
            # Quick check if download button is ready (icon button, no text)
            download_btn = self.page.locator('a[download], button[aria-label*="ownload" i], a[aria-label*="ownload" i]').first
            if download_btn.count() > 0 and download_btn.is_visible() and download_btn.is_enabled():
                self.last_activity = time.time()
                return True
            ...
            if self.current_task_start:
                ...
                if elapsed >= estimated_time:
                    result = download_btn.count() > 0 and download_btn.is_visible()
                    if result:
                        self.last_activity = time.time()
                    return result
                ...
                if not any_assigned and any_pending_unassigned:
                    ...
                    if no_ready_count % 20 == 1:  # Log every 10 seconds (20 * 0.5s)
                        ...
                        for wp in window_processors:
                            for tab in wp['tabs']:
                                try:
                                    gs_btn = tab.page.locator("button", has_text="Generate speech").last
                                    gen_btn = tab.page.locator("button", has_text="Generate").last
                                    dl_btn = tab.page.locator('a[download], button[aria-label*="ownload" i], a[aria-label*="ownload" i]').first
                                    # Also dump all button texts via JS for debugging
                                    all_btns = tab.page.evaluate("() => Array.from(document.querySelectorAll('button')).map(b => b.innerText.trim().substring(0,50)).filter(t => t.length > 0)")
                                    logger.warning(f"  {tab.tab_id}: 'Generate speech'={gs_btn.is_visible()}, 'Generate'={gen_btn.is_visible()}, Download={dl_btn.is_visible()}, busy={tab.is_busy}, errors={tab.error_count}")
                                    logger.warning(f"  {tab.tab_id}: ALL button texts: {all_btns}")
                                except Exception as e:
            ...
            while pending or task_assignments:
                ...
                for task_idx, tab in list(task_assignments.items()):
                    ...
                    try:
                        ...
                        # Check for download button (icon button, no text)
                        download_btn = page.locator('a[download], button[aria-label*="ownload" i], a[aria-label*="ownload" i]')
